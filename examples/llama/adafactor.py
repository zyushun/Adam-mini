...
from typing import Any, Dict, Tuple, Union, Iterable,Callable, Optional
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    if factored:
                        # changed here to reconcile with DTensor. This is the only change and is equivalent to the original code in torch_optimizer 
                        ...
                        # ).type_as(grad)
                        state['exp_avg_sq_row'] = torch.zeros_like(grad[0:, 0:1]).squeeze(1)
                        state['exp_avg_sq_col'] = torch.zeros_like(grad[0:1 , 0:]).squeeze(0)
...
class Adafactor_zhai(Optimizer):
    """Implements Adafactor algorithm.
    ...
    def __init__(
        ...
        super(Adafactor_zhai, self).__init__(params, defaults)
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    if factored:
                        # changed here to reconcile with DTensor. This is equivalent to the original code in torch_optimizer 
                        # state['exp_avg_sq_row'] = torch.zeros(
                        #     grad_shape[:-1]
                        # ).type_as(grad)
                        # state['exp_avg_sq_col'] = torch.zeros(
