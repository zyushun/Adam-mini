...
class DeepSpeedReMaxTrainer:
    ...
    def generate_experience(
        ...
        with torch.no_grad():
            ...
            ts = time.time()
            reward_output = self.reward_model.forward_value(
                seq, attention_mask, prompt_length=prompt_length_for_reward
            )
            reward_values = reward_output["values"].detach()
            reward_score = reward_output["chosen_end_scores"].detach()
    ...
    def compute_returns(
        self, prompts, kl_divergence, reward_score, reward_value, action_mask
    ):
        ...
        for j in range(batch_size):
            ...
            for i in reversed(range(start, ends[j])):
                if self.penalty.endswith("onestep"):
                    cumulative_kl = kl_divergence[j, i]
                else:
                    cumulative_kl += kl_divergence[j, i]

                if i == ends[j] - 1:
                    assert reward_value[j, i] == reward_score[j]
                returns[j, i] += cumulative_kl + cumulative_reward - reward_value[j, i]
        return returns
