...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    elif "lmsys/lmsys-chat-1m" in dataset_name:
        return raw_datasets.LMSYSChatDataset(
            output_path, seed, local_rank, dataset_name
        )
    elif "GAIR/lima" in dataset_name:
        return raw_datasets.LIMADataset(output_path, seed, local_rank, dataset_name)
    elif "shibing624/sharegpt_gpt4" in dataset_name:
        return raw_datasets.ShareGPT4Dataset(
            output_path, seed, local_rank, dataset_name
        )
    elif "theblackcat102/sharegpt-english" in dataset_name:
        return raw_datasets.ShareGPTenDataset(
            output_path, seed, local_rank, dataset_name
        )
    elif "openchat" in dataset_name:
        return raw_datasets.OpenChatDataset(output_path, seed, local_rank, dataset_name)
    elif "ultrafeedback_filtered" in dataset_name:
        return raw_datasets.UltrafeedbackDataset(
            output_path, seed, local_rank, dataset_name
        )
    elif "US_biased" in dataset_name:
        return raw_datasets.USBiasedDataset(output_path, seed, local_rank, dataset_name)
    ...
    if train_phase == 1:
        ...
        for i, tmp_data in enumerate(current_dataset):
            ...
            if chosen_sentence is not None:
                ...
                labels[(1 - chosen_token["attention_mask"]).to(torch.bool)] = (
                    IGNORE_INDEX
                )
        ...
        print(
            f"Creating dataset {raw_dataset.dataset_name_clean} for {train_phase=} size={len(chosen_dataset)} filtered = {filtered} total = {len(current_dataset)}"
        )
...
def create_prompt_dataset(
    ...
    """
    assert end_of_conversation_token == tokenizer.eos_token
